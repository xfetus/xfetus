...
class Discriminator(nn.Module):
    def __init__(self, img_size, ch):
        super().__init__()
        self.img_size = img_size
        self.ch = ch
        self.dwt = HaarTransform(1)
        self.conv = nn.ModuleList([
            DiscBlock(self.ch*8, self.ch*16),
            DiscBlock(self.ch*16, self.ch*16),
            DiscBlock(self.ch*16, self.ch*16),
            DiscBlock(self.ch*16, self.ch*16),
        ])
        self.skip = nn.ModuleList([
            FromRGB(256, downsample=False),
            FromRGB(512),
            FromRGB(512),
            FromRGB(512),
            FromRGB(512)
        ...
        self.final_conv = nn.Sequential(
            # EqualConv2d(self.ch*16, 1, 4, stride=1, padding=0, bias=False),
            # nn.LeakyReLU(0.2),
            EqualConv2d(512+1, 512, 3, stride=1, padding=1, bias=False)
        )
