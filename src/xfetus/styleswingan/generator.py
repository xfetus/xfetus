...
# modified last layer of the generator to fix white spots on images
    ...
    def __init__(self, latent_dim=512):
        ...
        self.swinlayers = nn.ModuleList([
            ...
            SwinTransformerLayer(dim=512, input_resolution=(8, 8), depth=2,
                                 num_heads=16, window_size=8, out_dim=512,
                                 upsample=True),
            SwinTransformerLayer(dim=512, input_resolution=(16, 16), depth=2,
                                 num_heads=16, window_size=8, out_dim=256,
                                 upsample=True),
            SwinTransformerLayer(dim=256, input_resolution=(32, 32), depth=2,
                                 num_heads=8, window_size=8, out_dim=128,
                                 upsample=True),
            SwinTransformerLayer(dim=128, input_resolution=(64, 64), depth=2,
                                 num_heads=4, window_size=8, out_dim=64,
                                 upsample=True),
            SwinTransformerLayer(dim=64, input_resolution=(128, 128), depth=2,
