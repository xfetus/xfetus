...
class FetalPlaneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load image
        img_name = os.path.join(self.root_dir, self.ref.iloc[idx, 0] + ".png")
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 100 == 0:
            ...
            if wandb_enabled:
                images = wandb.Image(
                    np.transpose(x[0, ...].detach().cpu().numpy(), (1, 2, 0)),
        ...
        if epoch % 1000 == 0:
            torch.save(
                image_pipe.unet.state_dict(),
                "128xfetal_half_noblur_tv" + str(epoch) + ".pth",
            )
            torch.save(
                optimizer.state_dict(),
                "128x_optim_fetal_half_noblur_tv" + str(epoch) + ".pth",
            )
